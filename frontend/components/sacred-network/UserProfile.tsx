...
import { MapPin, Link as LinkIcon, Calendar, Edit, Users, Heart, MessageCircle, Share, Settings, UserPlus, UserMinus, Phone, Video, Sparkles } from 'lucide-react'
...
export default function UserProfile({ userId, isOwnProfile = false }: UserProfileProps) {
  ...
  const [messengerThreadId, setMessengerThreadId] = useState<string | null>(null);
  ...
  const handleMessageUser = async () => {
    try {
      const { threadId } = await backend.messenger.start({ memberIds: [userId] });
      setMessengerThreadId(threadId);
  ...
  const handleStartGuidanceSession = () => {
    startCall(userId, 'voice', '', 'guidance');
  };
  ...
  return (
    <>
              ...
              <div className="flex items-end justify-between -mt-16 mb-4">
                ...
                <div className="flex items-center space-x-2">
                  {isOwnProfile ? (
                    ...
                    <>
                      {profile.experience_level === 'master' && (
                        <Button 
                          variant="outline"
                          className="border-purple-400 text-purple-300 hover:bg-purple-800/50"
      ...
      <MessengerDrawer 
        ...
        initialThreadId={messengerThreadId || undefined}
      />
    </>
  )
}
